...
rmax <- 2
w <- 0.1
plot(0, type="n", xlim=c(-rmax, rmax), ylim=c(-w, 1+w), axes=FALSE, ann=FALSE)
yh <- g(rmax, tau=tau)
...
draw_ellipse(0, 1, rmax, w, border="grey")
lines(-c(rmax, rmax), c(yh, 1), col="grey")
lines(c(rmax, rmax), c(yh, 1), col="grey")
```
...
lines(seq_len(length(br))-0.5, diff(c(0, pi_br)), col=3, type="b")
```
