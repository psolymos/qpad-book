...
curve(1-exp(-x*phi_Me0), xlim=c(0, 10), ylim=c(0, 1), col=4,
  xlab="Duration (min)", ylab=expression(p(t[J])), main=spp)
...
(b <- bsims_animate(a, vocal_rate=phi, mixture=mix))
b$vocal_rate
```
...
- Smaller singing rate species where p(10min) << 1

## Todo

- Figure out exercises
- Apply p as offset to estimate N
- compare ADp approach to N-mix (multi-visit)
- make a plot of inds x events, highlighting the 1st detections (ordered), tie in with ecdf
- explain conditioning in more depth
- explain: removal vs visits contrast (1st detection resets)
- add figure to constant mixture model similar to Me0
