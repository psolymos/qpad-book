...
based on adding/dropping terms (`drop1` is the function opposite to `add1`):

```{r-var_all}
mPstep <- step(glm(y ~ Open + Agr + UrbInd + SoftLin + Roads + 
  Decid + OpenWet + Conif + ConifWet + 
  OvernightRain + TSSR + DAY + Longitude + Latitude,
  data=x, family=poisson), trace=0) # use trace=1 to see all the steps
summary(mPstep)
```
...
that is a product of the two ($x_{12}=x_1 x_2$):
...
```
```{r regr-visreg2,fig.show = 'hold',out.width='33%'}
mep(mP5)
```
...
but the Chi-square test is still significant. Let's try different error distribution.

## Different error distributions

We will fit Negative Binomial, and 
